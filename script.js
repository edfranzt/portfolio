...
cards.forEach(card=>{card.addEventListener('click',e=>{if(e.target.closest('a'))return;const url=card.dataset.url;if(url)window.open(url,'_blank','noopener,noreferrer');});});
...
const grid=document.querySelector('.grid-bg');let ticking=false;
function updateParallax(){const y=window.scrollY;grid.style.transform=`translate3d(0,${y*.035}px,0)`;grid.style.setProperty('--glow-y',`${y*.07}px`);grid.style.setProperty('--glow-x',`${Math.sin(y*.002)*18}px`);ticking=false;}
